...
import functools
from typing import Any, Callable, Dict, Tuple, Optional, Type, Iterator
...
def train_step(
  ...
  def training_loss_fn(params):
    ...
    return loss, (new_model_state, logits, aux)

  compute_gradient_fn = jax.value_and_grad(training_loss_fn, has_aux=True)
  (train_cost, (new_model_state,
                logits, aux)), grad = compute_gradient_fn(train_state.params)
...
def eval_step(
        ...
        *,
        ...
        metrics_fn: MetricFn,
        debug: Optional[bool] = False
) -> Tuple[Dict[str, Tuple[float, int]], jnp.ndarray, Any]:
    """Runs a single step of training.
    ...
    logits, aux = flax_model.apply(
        variables, batch['inputs'], train=False, alpha=alpha, mutable=False, debug=debug)
    metrics = metrics_fn(logits, batch)
    eval_logs['eval_steps'] = aux
    return metrics, logits, eval_logs
...
def train(
  ...
  eval_step_pmapped = jax.pmap(
      functools.partial(
          eval_step,
          alpha=config.model.lac_config.test_alpha,
          flax_model=model.flax_model,
  ...
  def evaluate(train_state: train_utils.TrainState, step: int,
      ...
      for val_name, val_iter in valid_iter.items():
          ...
          for _ in range(steps_per_eval):
              ...
              e_metrics, _, eval_logs_steps = eval_step_pmapped(train_state, eval_batch)
              e_logs = jax.tree_util.tree_map(jax_utils.unreplicate, eval_logs_steps)
              eval_metrics.append(train_utils.unreplicate_and_get(e_metrics))
              extra_eval_logs.append(e_logs)
          eval_summary.update(
              train_utils.log_eval_summary(
                  step=step,
                  eval_metrics=eval_metrics,
                  writer=writer,
                  extra_eval_summary=jax.tree_util.tree_map(jax.device_get,
                                                            extra_eval_logs),
                  prefix=val_name))
      del eval_metrics
      writer.flush()
